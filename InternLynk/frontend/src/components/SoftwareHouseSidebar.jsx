...
export default function SoftwareHouseSidebar() {
  ...
  const email = user?.email || 'software@internlynk.com'
  const pictureUrl = profile?.profile_picture ? getProfilePictureUrl(profile.profile_picture) : null
  const userName = profile?.organization_name || profile?.full_name || user?.email?.split('@')[0] || 'Software House'
  const displayInitials = userName ? userName[0].toUpperCase() : (user?.email || 'S')[0]?.toUpperCase()
            ...
            {pictureUrl ? (
              <div className="relative">
                ...
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center border-2 border-blue-200 shadow-sm hidden">
                  <span className="text-sm font-semibold text-white">{displayInitials}</span>
                </div>
              </div>
            ) : (
